        ...
        switch (token) {
            ...
            .String => |stringToken| {
                ...
                switch (stringToken.escapes) {
                    .None => return Value{ .String = try mem.dupe(allocator, u8, source_slice) },
                    .Some => |some_escapes| {
...
pub const ParseErrorInfo = union(enum) {
    None,
    MissingField: struct {
        name: []const u8,
        pos: usize,
    ...
    switch (@typeInfo(T)) {
        ...
        .Optional => |optionalInfo| {
            if (token == .Null) {
                ...
                return try parseInternal(optionalInfo.child, token, tokens, options, parseErrorInfo);
            }
        },
        .Enum => |enumInfo| {
            switch (token) {
            ...
            inline for (structInfo.fields) |field, i| {
                if (!fields_seen[i]) {
                    if (field.default_value) |default| {
                        ...
                        if (parseErrorInfo) |info| {
                            info.* = .{ .MissingField = .{ .name = field.name, .pos = tokens.i - 1 } };
                        }
        ...
        .Array => |arrayInfo| {
            switch (token) {
                .ArrayBegin => {
                    ...
                    while (i < r.len) : (i += 1) {
                        r[i] = try parse(arrayInfo.child, tokens, options, parseErrorInfo);
                    }
                    ...
                    switch (token) {
                        .ArrayBegin => {
                            ...
                            while (true) {
                                ...
                                const v = try parseInternal(ptrInfo.child, tok, tokens, options, parseErrorInfo);
...
pub fn parseFree(comptime T: type, value: T, options: ParseOptions) void {
    if (comptime std.meta.trait.hasFn("jsonParseFree")(T)) {
        return value.jsonParseFree(options);
    }
    if (comptime std.meta.trait.hasFn("jsonParse")(T)) {
        @compileError(@typeName(T) ++ " defines jsonParse; it must also define jsonParseFree");
    }
...
pub const Parser = struct {
    ...
    pub fn parsePartial(p: *Parser, input: []const u8) !ValueTree {
        var s = TokenStream.init(input);
        ...
        while (try s.next()) |token| {
            debug.warn("token: {}\n", .{token});
            const early_return = try p.transition(&arena.allocator, input, s.i - 1, token);
            if (early_return) {
                break;
            }
        }
    ...
    fn transition(p: *Parser, allocator: *Allocator, input: []const u8, i: usize, token: Token) !bool {
        switch (p.state) {
